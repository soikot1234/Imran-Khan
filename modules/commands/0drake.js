...
if(!fs.existsSync(__dirname+'/cache/dipto.ttf')) { 
      let getfont = (await axios.get(`https://api.popcat.xyz/drake?text1=amongus&text2=amogus`, { responseType: "arraybuffer" })).data;
       fs.writeFileSync(__dirname+"/cache/dipto.ttf", Buffer.from(getfont, "utf-8"));
    };
  let baseImage = await loadImage(pathImg);
  let canvas = createCanvas(baseImage.width, baseImage.height);
  let ctx = canvas.getContext("2d");
  ctx.drawImage(baseImage, 0, 0, canvas.width, canvas.height);
  Canvas.registerFont(__dirname+`/cache/dipto.ttf`, {
        family: "SVN-Arial 2"
    });
